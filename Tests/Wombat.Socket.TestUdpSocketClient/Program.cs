...
namespace Wombat.Socket.TestUdpSocketClient
{
    class Program
    {
        static UdpSocketClient _client;
        static ILogger _logger;
        static UdpSocketClientConfiguration _config;
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                _config = new UdpSocketClientConfiguration();
                
                // UDP特有配置
                _config.ConnectedMode = true;                      // 使用Connected模式
                ...
                _config.HeartbeatInterval = TimeSpan.FromSeconds(15);   // 每15秒发送一次心跳
                _config.HeartbeatTimeout = TimeSpan.FromSeconds(30);    // 30秒未收到心跳则超时
                _config.MaxMissedHeartbeats = 2;                        // 允许最多2次心跳丢失

                IPEndPoint remoteEP = new IPEndPoint(IPAddress.Parse("127.0.0.1"), 22222);
                _client = new UdpSocketClient(remoteEP, new SimpleEventDispatcher(), _config);
                // 设置日志记录器
                ...
                Console.WriteLine("UDP client has connected to server [{0}].", remoteEP);
                    ...
                    try
                    {
                        ...
                        Task.Run(async () =>
                        {
                            ...
                            else if (text == "big10k")
                            {
                                ...
                                Console.WriteLine("Client [{0}] send text -> [{1} Bytes].", _client.LocalEndPoint, text.Length);
                                Thread.Sleep(2000);
                                // UDP客户端没有ReadAllBytes方法，因为UDP是无连接的
                ...
                _client.Close().ConfigureAwait(false).GetAwaiter().GetResult();
                Console.WriteLine("UDP client has disconnected from server [{0}].", remoteEP);
            }
            catch (Exception ex)
            {
